...
def readfile(filename):
    ...
    MOD -> [ ['EU', 'DET', 'B-ORG'], ['rejects', 'DET', 'O'], ['German', 'DET', 'B-MISC'], ['call', 'DET', 'O'], ...]
    '''
    f = open(filename)
    sentences = []
    sentence = []
    for line in f:
        if len(line) == 0 or line.startswith('-DOCSTART') or line[0] == "\n":
            ...
            continue
        splits = line.split(' ')
        sentence.append([splits[0], splits[1], splits[-2]])
...
def createMatrices(sentences, word2Idx, label2Idx, case2Idx, char2Idx, postag2Idx):
    ...
    for sentence in sentences:
        ...
        for word, postag, char, label in sentence:
            if word == "":
                print("ERRROOOOOOR")
            ...
            if postag=='' or postag=='_':
                postag = 'X'
            postagIndices.append(postag2Idx[postag])

        dataset.append([wordIndices, caseIndices, charIndices, labelIndices, postagIndices])
...
def iterate_minibatches(dataset, batch_len):
    ...
    for i in batch_len:
        ...
        yield np.asarray(labels), np.asarray(tokens), np.asarray(caseing), np.asarray(char), np.asarray(postags)
...
def addCharInformation(Sentences):
    for i, sentence in enumerate(Sentences):
        for j, data in enumerate(sentence):
            ...
            Sentences[i][j] = [data[0], data[1], chars, data[2]]
    return Sentences
